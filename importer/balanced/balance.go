...
//	  | Chunk 1 |   | Chunk 2 |   | Chunk 3 |
//	  +=========+   +=========+   + - - - - +
func LayoutI(db h.Helper) (ipld.Node, error) {
	if db.Done() {
	...
	for depth := 1; !db.Done(); depth++ {
		...
		// it will become "old").
		root, fileSize, err = fillNodeRecI(db, newRoot, depth)
		if err != nil {
			return nil, err
		}
	}
...
func fillNodeRecI(db h.Helper, node *h.FSNodeOverDag, depth int) (filledNode ipld.Node, nodeFileSize uint64, err error) {
	if depth < 1 {
	...
	for node.NumChildren() < db.Maxlinks() && !db.Done() {
		...
		if depth == 1 {
			...
			childNode, childFileSize, err = fillNodeRecI(db, nil, depth-1)
			if err != nil {
				return nil, 0, err
			}
		}
